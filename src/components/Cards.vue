<template>
  <div
    class="card-container flex flex-wrap h-auto mt-8 mb-8 text-center text-emiyablack"
  >
    <div
      class="lg:w-1/3 sm:w-full max-w-m rounded overflow-hidden shadow hover:shadow-lg pt-4 pb-4"
    >
      <p class="font-bold text-xl mb-4">Why Emiya Consulting</p>
      <ul class="pl-16 pr-16">
        <li class="mb-4 pb-4 border-b border-solid border-gray-100">
          Services designed for your small business or individual needs, no
          one-size-fits-all solutions
        </li>
        <li class="mb-4 pb-4 border-b border-solid border-gray-100">
          End-to-end support: We manage design, development, deployment and
          hosting
        </li>
        <li class="mb-4 pb-4 border-b border-solid border-gray-100">
          Simple: no overcomplicated, costly solutions and a range of additional
          tech consulting services for small business and individuals including
          network, cloud, BCP/DR strategies, and general tech support
        </li>
        <li>
          Business experience: 25+ Years at an F500 corporation; you don't need
          just a coder, but a partner and someone who understands real-world
          business
        </li>
      </ul>
    </div>
    <div
      class="lg:w-1/3 sm:w-full max-w-m rounded overflow-hidden shadow hover:shadow-lg pt-4 pb-4"
    >
      <p class="font-bold text-xl mb-4">Web Design and Development</p>
      <ul class="pl-16 pr-16">
        <li class="mb-4 pb-4 border-b border-solid border-gray-100">
          Responsive, accessible static websites as well as dynamic web apps,
          Single Page Apps, and Electron desktop apps
        </li>
        <li class="mb-4 pb-4 border-b border-solid border-gray-100">
          Modern front-end frameworks and libraries including Vue.js, React.js,
          Bootstrap, Material-UI / Material Design, and Tailwind
        </li>
        <li class="mb-4 pb-4 border-b border-solid border-gray-100">
          Security, search-engine optimization (SEO), and disaster recovery at
          every step
        </li>
        <li>
          Back-end APIs / web services and databases using Node.js or Google's
          Go language, connected to MongoDB, MySQL, MS SQL, Firebase /
          Firestore, or PostgreSQL
        </li>
      </ul>
    </div>
    <div
      class="lg:w-1/3 sm:w-full max-w-m rounded overflow-hidden shadow hover:shadow-lg pt-4 pb-4"
    >
      <p class="font-bold text-xl mb-4">Mobile iOS App Development</p>
      <ul class="pl-16 pr-16">
        <li class="mb-4 pb-4 border-b border-solid border-gray-100">
          Apps built for iPhone and iPad using native development tools,
          providing security and performance on Apple devices; Android device
          support is coming soon
        </li>
        <li class="mb-4 pb-4 border-b border-solid border-gray-100">
          Back-ends built to code once and deploy anywhere so they can be shared
          by web and mobile apps; performance and security are prioritized
        </li>
        <li class="mb-4 pb-4 border-b border-solid border-gray-100">
          Cloud first: Specializations in Google Cloud Platform, Firebase, and
          AWS allow for fast deployment and easy updates to your app APIs
        </li>
        <li>
          Your apps are monitored, updated and maintained for reasonable
          maintenance costs
        </li>
      </ul>
    </div>
  </div>
</template>

<script>
export default {
  name: "Cards",
};
</script>

<style scoped></style>
